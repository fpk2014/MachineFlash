...
var QUIT_FIND_OUT_TIME = 80;               //寻找时间点间隔，耗时*20
...
var CLICK_MACHINE_TIME   =  100;
var CLICK_FINAL_DAY_TIME =  100;
var MACHINE_NUMBER = 4;                    //机器数量
//var IDEAL_MACHINE = 3;                     //选择的机器,单位：点
//var IDEAL_TIME = 10;                     //选择的时间段,单位：点
var PRINT_LOG = false;
...
//var IDEAL_MACHINE = GM_getValue ("machineID", "");
var IDEAL_TIME    = GM_getValue ("timeID",    "");
var IDEAL_MACHINE = GM_getValue ("machineID", "");
...
if (!encKey) {
    ...
    YOUR_JOB     = YOUR_ID = IDEAL_TIME = IDEAL_MACHINE = "";   // New key makes prev stored values (if any) unable to decode.
}
GM_registerMenuCommand ("修改作业名称", changeUsername);
GM_registerMenuCommand ("修改手机号码", changePassword);
GM_registerMenuCommand ("修改机器号"  , changeMachine);
GM_registerMenuCommand ("修改时间点"  , changeTime);
...
async function All_Process() {
    try{
        ...
        //准备工作:跳转到下一步骤
        await Sleep(QUIT_FIND_OUT_TIME);
        ...
        Input_Name_Or_PhoneID('input',  "id",  "jobName",        "value", YOUR_JOB);                //输入作业名称
        Input_Name_Or_PhoneID('input',  "id",  "bookingUserTel", "value", YOUR_ID );                //输入手机号码
        ...
        await Sleep(QUIT_FIND_OUT_TIME);  //等待加载完成
        Click_Final_Day('li',    "onclick").click();                                                         //选择最后一天
        await Sleep(QUIT_FIND_OUT_TIME*3); //等待加载完成
        if(button.innerHTML==="Start"){return;}
        if(Click_Time('a',   "class", "no_subscribe", "name", parseInt(IDEAL_TIME))===true){return;}         //选择某个时间点
        
        
        await Sleep(WAIT_LOAD_TIME);    //等待加载完成
        //快速遍历所有时间点
        for(var i=1; i<=MACHINE_NUMBER; i++){
            await Sleep(QUIT_FIND_OUT_TIME*i); //等待加载完成
        ...
        for(var i=1; i<=MACHINE_NUMBER; i++){
            ...
            Print("test machine "+i);
            Click_Machine('a',       "name", i).click();                                             //选择i号机器
            await Sleep(Click_Machine_TIME*i);  //等待加载完成
            Click_Final_Day('li',    "onclick").click();                                             //选择最后一天
            await Sleep(Click_Final_Day_TIME*i); //等待加载完成
